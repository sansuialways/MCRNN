...
class DataGenerator(object):
    def __init__(self, dataset='foa', nb_ch = 4, feat_label_dir='', is_eval=False, split=1, batch_size=32, seq_len=128, shuffle=True, per_file=False, 
                 feat_type='mel', doa=None, seed=1, onlyphase=False, trial=None):
        # MY ADDITION
        ...
        self._feat_cls = cls_feature_class.FeatureClass(feat_label_dir=feat_label_dir, dataset=dataset, is_eval=self._is_eval, doa=doa)
    ...
    def get_data_sizes(self):
        ...
        #print('channel is {}'.format(self._2_nb_ch))
        # if self._is_eval:
        #     label_shape = None
        # else:
        if not self.doa or self.doa == 'reg':
            label_shape = [
            ...
            label_shape = [
                (self._batch_size, self._seq_len, self._nb_classes),
                (self._batch_size, self._seq_len, self._nb_classes, self._nb_azi+self._nb_ele)
            ]
    ...
    def _get_filenames_list_and_feat_label_sizes(self):
        ...
        for filename in os.listdir(self._feat_dir):
            if filename[6].isdigit():
                file_split_num = int(filename[5]+filename[6])
            else:
                file_split_num = int(filename[5])
            if file_split_num in self._splits: # check which split the file belongs to
                self._filenames_list.append(filename)
        #print('the feature dir is {},length is {}'.format(self._feat_dir,len(self._filenames_list)))
        ...
        #print('sed feat_len is {},channel is {}'.format(self._feat_len,self._2_nb_ch))
        ...
        if not self.doa or self.doa == 'reg':
            ...
            self._doa_len = self._label_len - 1     # 1 is for sed
    ...
    def generate(self):
        while 1:
            ...
            file_cnt = 0
            if self.doa == 'reg' or not self.doa:
                for i in range(self._nb_total_batches):
                    ...
                    #print('the first feature length is {}'.format(self._batch_seq_len))
                    #print(feat.shape[0], feat.shape[1], feat.shape[2], feat.shape[3])                    # Get azi/ele in radians
                ...
                for i in range(self._nb_total_batches):
                    ...
                    while len(self._circ_buf_feat) < self._batch_seq_len:
                        ...
                        if self._per_file:
                            ...
                            extra_labels = np.zeros((extra_frames, temp_label.shape[1], temp_label.shape[2]))
                            #extra_labels[:, self._nb_classes:2 * self._nb_classes] = self._default_azi
                            #extra_labels[:, 2 * self._nb_classes:] = self._default_ele
                    ...
                    # Read one batch size from the circular buffer
                    feat = np.zeros((self._batch_seq_len, self._feat_len * self._2_nb_ch))
                    label = np.zeros((self._batch_seq_len, self._nb_classes, self._label_len))
                    for j in range(self._batch_seq_len):
                        feat[j, :] = self._circ_buf_feat.popleft()
                        label[j, :] = self._circ_buf_label.popleft()
                    feat = np.reshape(feat, (self._batch_seq_len, self._feat_len, self._2_nb_ch))
                    ...
                    label = [
                        label[:, :, :, 0],                       # SED labels                        [batch, seq_len, 11]
                        label[:, :, :, 1:]                       # DOA labels in the form of one-hot [batch, seq_len, 11, 36*9]
                        ]
