...
class FeatureClass:
    def __init__(self, dataset_dir='', feat_label_dir='', dataset='foa', is_eval=False, doa=None):
        """
        ...
        self._dataset_combination = '{}_{}'.format(dataset, 'eval' if self._is_eval else 'dev')
        #print('the dataset combination is {}########################################################'.format(self._dataset_combination))
        #print('this is eval feat {}'.format(self._dataset_combination))
        self._aud_dir = os.path.join(self._dataset_dir, self._dataset_combination)

        self._desc_dir = os.path.join(self._dataset_dir, 'metadata_eval') if self._is_eval else os.path.join(self._dataset_dir, 'metadata_dev')
    ...
    def read_desc_file(self, desc_filename, in_sec=False):
        desc_file = {
            'class': list(), 'start': list(), 'end': list(), 'ele': list(), 'azi': list(), 'dis': list()
        }
        ...
        for line in fid:
            ...
            desc_file['dis'].append(int(split_line[5]))
        fid.close()
        return desc_file
    ...
    def _get_doa_labels_regr(self, _desc_file):
        ...
        dis_label = np.ones((self._max_frames, len(self._unique_classes)))
        for i, ele_ang in enumerate(_desc_file['ele']):
            ...
            if (azi_ang >= self._azi_list[0]) & (azi_ang <= self._azi_list[-1]) & \
                ...
                dis_label[start_frame:end_frame + 1, class_ind] = dis_ang
            else:
                print('bad_angle {} {}'.format(azi_ang, ele_ang))
        doa_label_regr = np.concatenate((azi_label, ele_label, dis_label), axis=1)
    ...
    def get_normalized_feat_dir(self):
        ...
        return os.path.join(
            '../task3-dataset/',
            'feat_label_mel/{}_norm'.format(self._dataset_combination)
        )

    def get_unnormalized_feat_dir(self):
        return os.path.join(
            self._dataset_dir,
            'feat_label_mel/{}'.format(self._dataset_combination)
        )
