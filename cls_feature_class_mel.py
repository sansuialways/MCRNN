...
class FeatureClass:
    def __init__(self, dataset_dir='', feat_label_dir='', dataset='foa', is_eval=False):
        ...
        self._desc_dir =  os.path.join(self._dataset_dir, 'metadata_{}'.format('eval' if is_eval else 'dev'))#None if is_eval else
        ...
        self._win_len_s = 2 * self._hop_len_s
        ...
        # MY ADDITION FOR MEL CALCULATION
        self._nmel = 128                        # Number of mel bands used
    ...
    def _spectrogram(self, audio_input):
        ...
        spectra = np.zeros((nb_bins, self._max_frames, _nb_ch), dtype=complex)
    ...
    def _spectrogram(self, y, window, center=True, spectrogram_type='magnitude'):
        """
        Returns
        -------
        ...
        if spectrogram_type == 'magnitude':
            return np.abs(librosa.stft(y + self.eps,
                                          n_fft=self._nfft,
    ...
    def _mel(self, data):
        _nb_ch = data.shape[1]
        window = self._window_function(N=self._win_len, window_type='hamming_asymmetric')
        ...
        feature_matrix = np.zeros((self._max_frames, self._nmel, _nb_ch))
        for ch in range(_nb_ch):
            ...
            mel_spectrum = np.log(mel_spectrum + self.eps)

            mel_spectrum = mel_spectrum.T
            feature_matrix[:, :, ch] = mel_spectrum[:self._max_frames, :]
    ...
    def _extract_spectrogram_for_file(self, audio_filename):
        ...
        assert fs == self._fs
        audio_mel = self._mel(audio_in)
        # print('\t{}'.format(audio_spec.shape))
        np.save(os.path.join(self._feat_dir, '{}.npy'.format(audio_filename.split('.')[0])), audio_mel.reshape(self._max_frames, -1))
    ...
    def get_clas_labels_for_file(self, _desc_file):
        ...
        :param _desc_file: csv file
        :return: _labels: matrix of SELD labels of dimension [nb_frames, nb_classes, nb_azi*nb_ele], which is 1 for active sound event and location else zero
        """
    ...
    def get_label_dir(self):
        if self._is_eval:
            return os.path.join(
                    self._feat_label_dir, 'temp', '{}_label'.format(self._dataset_combination)
                )            
        else:
            return os.path.join(
                    self._feat_label_dir, 'temp', '{}_label'.format(self._dataset_combination)
